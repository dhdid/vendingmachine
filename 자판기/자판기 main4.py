...
coin=0            # 500원, 100원 짜리 동전을 반환할 수 있는지 여부를 판단하기 위한 변수 
a = 1             #거스름돈 없을시 루프 탈출 위한 변수
...
class Money:
    ...
    def moneyinput():                             #돈을 투입 받고 투입된 돈을 계산
        ...
        charge+=inp_money
        return inp_money, charge, ba_1000, ba_500, ba_100
    ...
    def chargeDiv():                             # 잔액 500원 양과 100원 양을 계산
        global r500, r100, charge
        a=charge//500
        r500=a
        r100=(charge-a*500)//100
        return r100, r500

    def Jugement():                             # 잔액 반환 가능 여부 판단
        global charge, r500, r100, ba_500, ba_100, coin
        if ba_500 >= r500 and ba_100 >= r100 :
            coin=0
            return coin
        elif ba_500 < r500 and ba_100 >= r100 :
            coin=1
            return coin
        else:
            coin=2
...
def main():
    ...
    if k=="2022156030":                       #관리자 모드 아이디, 아이디를 옳게 입력시 관리자 모드로 진입
        ...
        if retorplus == "반환" or int(retorplus) == 1:
            ...
            if coin == 0 or 1 :
                Money.outCh
                Money.chargecomput(charge)
                print("500원", r500,"개가 100원",r100,"개가 반환되었습니다.")
            else:                           #잔액 반환이 안될시 실행
                print("사진을 찍어 '070-0000-0000으로 연락주세요")
                global a
                a = 2
                return a
...
while a == 1 :
    main()                                   #잔액이 반환 안될시 루프 탈출


################## 추가
 # 구매를 원하지 않을 시 > 항상 반환    추가구매 투입된돈 x > 반환> 초기화면     음료 화면 